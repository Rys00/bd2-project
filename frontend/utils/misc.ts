...
export async function makeBackendRequest<RetType>(
    endpoint: string, method: 'GET'|'POST'|'DEL'|'PUT'|'PATCH',
    // eslint-disable-next-line @typescript-eslint/no-explicit-any
    requestData?: any, dispatch?: AppDispatch) {
  let jwt = '';
  try {
  ...
  try {
    res = await fetch(
        `${process.env.NEXT_PUBLIC_BACKEND_URL}/${endpoint}`, config);
  } catch (error) {
...
export async function makeRequest<
    ...
    func: Func, args: Parameters<Func>, dispatch?: AppDispatch):
...
export async function transferWithJSON<
  ...
  return JSON.parse(await invokeTransferWithJSON(func, JSON.stringify(args))) as
      ReturnType<Func>;
}
...
export function hashPassword(password: string) {
  return new Promise((resolve: (value: Buffer) => void, reject) => {
    ...
    crypto.randomBytes(config.saltBytes, (err, salt) => {
      ...
      // hash password
      crypto.pbkdf2(
          password, salt, config.iterations, config.hashBytes, config.digest,
          (err, hash) => {
            if (err) {
              return reject(err);
            }
...
export function verifyPassword(password: string, combined: Buffer) {
  return new Promise((resolve: (value: boolean) => void) => {
    ...
    crypto.pbkdf2(
        password, salt, iterations, hashBytes, config.digest, (err, verify) => {
          if (err) {
